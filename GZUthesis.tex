...
\begin{document}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item 增加模板参数，调整 \texttt{Times New Roman},\texttt{ Arial}, \texttt{华文楷体}字体的搜索策略，优化编译
            \item 其他优化
        \end{itemize}
      \item[1.1.5] (2026-04-29) 调整论文模板排版与校徽资源，添加盲审回复模版
      \begin{itemize}
          \item 更新论文模板排版细节与校徽资源，补充盲审意见修改回复模版
          \item 调整终稿与盲审封面标题年份间距，优化答辩信息页留白、行距和委员会名单列距
          \item 统一章节、节和小节标题使用黑体样式，细化标题前后间距，避免标题与正文或目录项挤压
          \item 调整目录中章、节、小节的缩进与引导点样式，移除全局禁用 \cs{addvspace} 的处理
          \item 启用中文主字体伪粗体和伪斜体支持，改善宋体生僻字在加粗场景下的显示兼容性
          \item 优化答辩委员会名单三列间距，降低答辩信息页行距，改善人员信息较多时的版面稳定性
          \item 示例终稿增加图目录、表目录和长文本测试内容，将生僻字测试移入正文示例章节
          \item 清理示例终稿中的临时测试文本，补充三级标题示例用于验证标题和目录样式
      \end{itemize}
    ...
    \section{已知问题}
    由于平台字体限制，除windows 平台外，其他平台不能按照模板规范显示，例如 linux系统使用的不是Sim*字体，部分扩展区文字显示乱码。 在非Windows系统，模板默认不适用 \texttt{Times New Roman}, \texttt{Arial}, \texttt{华文楷体}等字体。
    若系统中有\texttt{Times New Roman}, \texttt{Arial} 字体 请使用  包选项 \textbf{tnra} 启用。

    中文字体由 \textbf{ctex}提供，模板并未修改。

    盲审版首页 \textbf{XX 届X士研究生学位论文} 字样为华文行楷（STXINGKA）。在非Windows 系统下， 若系统有该字体或文档目录有STXINGKA.TTF文件，则使用 包选项 \textbf{hwxk} 启用华文行楷， 否则使用楷体。
    ...
    \begin{function}[added=2025-01-02]{hwxk}
        \begin{syntax}
            hwxk
        \end{syntax}
        若使用 hwxk 参数，则尝试在系统或者文档当前目录下搜索 \texttt{华文楷体} 字体并使用，该参数仅在非windows 系统下有效。
        \begin{description}
    ...
    \begin{function}[added=2025-01-02]{tnra}
        \begin{syntax}
            tnra
        \end{syntax}
        若使用 tnra 参数，则尝试在系统搜索 \texttt{Arial} 和 \texttt{Times New Roman} 字体并使用，该参数仅在非windows 系统下有效。
        \begin{description}
    ...
    \begin{function}[added=2024-08-18]{\addexpert}
        ...
        \end{syntax}
        该命令为答辩专家列表添加一个专家信息，可以重复调用，只会追加信息。单位、姓名、职称会分列排版，便于对齐。
    \end{function}
    ...
    \begin{function}[added=2024-08-18]{\chairman}
        ...
        \end{syntax}
        该命令设置答辩主席，默认留空。单位、姓名、职称会分列排版，职称后自动追加“（主席）”。
    \end{function}
