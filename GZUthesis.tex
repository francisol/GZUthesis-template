...
\begin{document}
    ...
    \section{已知问题}
    由于平台字体限制，除windows 平台外，其他平台不能按照模板规范显示，例如 linux系统默认使用的不是Sim*字体，部分扩展区文字可能显示乱码。若系统中有对应字体，请使用包选项 \textbf{windows} 启用 Windows 字体模式；该选项会自动尝试启用 \texttt{Times New Roman}, \texttt{Arial}, \texttt{华文行楷}等字体。
    ...
    \begin{function}[added=2024-08-19]{windows}
        \begin{syntax}
            windows
        \end{syntax}
        若使用windows参数，则尝试在系统或者文档当前目录下搜索 Windows 常用论文排版字体并使用，同时自动启用 \textbf{hwxk} 与 \textbf{tnra} 字体检查。
        \begin{description}
...
\abbrdef{生成对抗网络}{Generative Adversarial Network}{GAN}
    ...
    \begin{function}[added=2026-04-29]{\abbritem}
        \begin{syntax}
            ...
            \cs{abbritem}\oarg{分组}\marg{中文名称}\marg{英文全称}\marg{缩写}
        \end{syntax}
        在当前位置输出“中文名称（英文全称，缩写）”，同时登记到缩略语表。可选分组默认为 A。
    \end{function}
...
\nomdef{深度学习}{Deep Learning}
    ...
    \begin{function}[added=2026-04-29]{\nomitem}
        \begin{syntax}
            ...
            \cs{nomitem}\oarg{分组}\marg{中文术语}\marg{英文术语}
        \end{syntax}
        在当前位置输出“中文术语（英文术语）”，同时登记到术语表。可选分组默认为 N。
    \end{function}

    \begin{function}[added=2026-04-29]{\termitem}
        \begin{syntax}
            \cs{termitem}\marg{中文术语}\marg{解释}\\
            \cs{termitem}\oarg{分组}\marg{中文术语}\marg{解释}
        \end{syntax}
        在当前位置输出“中文术语”，同时登记到术语表。可选分组默认为 N。
    \end{function}
